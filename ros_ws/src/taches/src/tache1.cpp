...
void Tache1::FinTraitementProduit(int numTable)
{
...
void Tache1::PositionRobotCallback(const std_msgs::Int32::ConstPtr& msg)
{
	position = msg->data;
}
...
void Tache1::BrasRobotCallback(const std_msgs::Int32::ConstPtr& msg)
{
	bras = msg->data;
}
...
void Tache1::PinceRobotCallback(const std_msgs::Int32::ConstPtr& msg)
{
	pince = msg->data;
}
...
void Tache1::TraitementProduitCallback(const std_msgs::Int32::ConstPtr& msg)
{
	produit=msg->data;
...
void Tache1::init(ros::NodeHandle noeud, std::string executionPath)
{
	...
	//Subscribers
	robotPosition = noeud.subscribe("/robot/PositionRobot1", 10, &Tache1::PositionRobotCallback, this);
	
	robotBras = noeud.subscribe("/robot/BrasRobot1",10,&Tache1::BrasRobotCallback,this);

	robotPince = noeud.subscribe("/robot/PinceRobot1",10,&Tache1::PinceRobotCallback,this);

	posteProduit = noeud.subscribe("/P1_Tache/ProduitATraiter",10,&Tache1::TraitementProduitCallback,this);


	//Publishers	
	pub_traitementProduitTache = noeud.advertise<std_msgs::Int32>("/Tache_P1/Tache1Finie",10);

	pub_retourTraitement = noeud.advertise<std_msgs::Int32>("/commande/Simulation/ProduitTraitement1",10);
...
void Tache1::initProduct(int nJobtime, int pNumber)
{
