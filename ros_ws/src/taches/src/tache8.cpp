...
void Tache8::FinTraitementProduit(int numTable)
{
...
void Tache8::PositionRobotCallback(const std_msgs::Int32::ConstPtr& msg)
{
	position = msg->data;
}
...
void Tache8::BrasRobotCallback(const std_msgs::Int32::ConstPtr& msg)
{
	bras = msg->data;
}
...
void Tache8::PinceRobotCallback(const std_msgs::Int32::ConstPtr& msg)
{
	pince = msg->data;
}
...
void Tache8::TraitementProduitCallback(const std_msgs::Int32::ConstPtr& msg)
{
	produit=msg->data;	
...
void Tache8::init(ros::NodeHandle noeud, std::string executionPath)
{
	...
	//Subscribers
	robotPosition = noeud.subscribe("/robot/PositionRobot4", 10, &Tache8::PositionRobotCallback, this);
	
	robotBras = noeud.subscribe("/robot/BrasRobot4",10,&Tache8::BrasRobotCallback,this);

	robotPince = noeud.subscribe("/robot/PinceRobot4",10,&Tache8::PinceRobotCallback,this);

	posteProduit = noeud.subscribe("/P8_Tache/ProduitATraiter",10,&Tache8::TraitementProduitCallback,this);


	//Publishers	
	pub_traitementProduitTache = noeud.advertise<std_msgs::Int32>("/Tache_P8/Tache8Finie",10);

	pub_retourTraitement = noeud.advertise<std_msgs::Int32>("/commande/Simulation/ProduitTraitement8",10);
...
void Tache8::initProduct(int nJobtime, int pNumber)
{
