...
void Tache7::FinTraitementProduit(int numTable)
{
...
void Tache7::PositionRobotCallback(const std_msgs::Int32::ConstPtr& msg)
{
	position = msg->data;
}
...
void Tache7::BrasRobotCallback(const std_msgs::Int32::ConstPtr& msg)
{
	bras = msg->data;
}
...
void Tache7::PinceRobotCallback(const std_msgs::Int32::ConstPtr& msg)
{
	pince = msg->data;
}
...
void Tache7::TraitementProduitCallback(const std_msgs::Int32::ConstPtr& msg)
{
	produit=msg->data;	
...
void Tache7::init(ros::NodeHandle noeud, std::string executionPath)
{
	...
	//Subscribers
	robotPosition = noeud.subscribe("/robot/PositionRobot4", 10, &Tache7::PositionRobotCallback, this);
	
	robotBras = noeud.subscribe("/robot/BrasRobot4",10,&Tache7::BrasRobotCallback,this);

	robotPince = noeud.subscribe("/robot/PinceRobot4",10,&Tache7::PinceRobotCallback,this);

	posteProduit = noeud.subscribe("/P7_Tache/ProduitATraiter",10,&Tache7::TraitementProduitCallback,this);


	//Publishers	
	pub_traitementProduitTache = noeud.advertise<std_msgs::Int32>("/Tache_P7/Tache7Finie",10);

	pub_retourTraitement = noeud.advertise<std_msgs::Int32>("/commande/Simulation/ProduitTraitement7",10);


	//Définition des noms des signaux
	table = "Table#10_color";
...
void Tache7::initProduct(int nJobtime, int pNumber)
{
