...
void Tache6::FinTraitementProduit(int numTable)
{
...
void Tache6::PositionRobotCallback(const std_msgs::Int32::ConstPtr& msg)
{
	position = msg->data;
}
...
void Tache6::BrasRobotCallback(const std_msgs::Int32::ConstPtr& msg)
{
	bras = msg->data;
}
...
void Tache6::PinceRobotCallback(const std_msgs::Int32::ConstPtr& msg)
{
	pince = msg->data;
}
...
void Tache6::TraitementProduitCallback(const std_msgs::Int32::ConstPtr& msg)
{
	produit=msg->data;	
...
void Tache6::init(ros::NodeHandle noeud, std::string executionPath)
{
	...
	//Subscribers
	robotPosition = noeud.subscribe("/robot/PositionRobot3", 10, &Tache6::PositionRobotCallback, this);
	
	robotBras = noeud.subscribe("/robot/BrasRobot3",10,&Tache6::BrasRobotCallback,this);

	robotPince = noeud.subscribe("/robot/PinceRobot3",10,&Tache6::PinceRobotCallback,this);

	posteProduit = noeud.subscribe("/P6_Tache/ProduitATraiter",10,&Tache6::TraitementProduitCallback,this);


	//Publishers	
	pub_traitementProduitTache = noeud.advertise<std_msgs::Int32>("/Tache_P6/Tache6Finie",10);

	pub_retourTraitement = noeud.advertise<std_msgs::Int32>("/commande/Simulation/ProduitTraitement6",10);
...
void Tache6::initProduct(int nJobtime, int pNumber)
{
