...
void Robot1::EnvoyerRobot(int numposition)
{
	...
	{
		ROS_INFO("Position predefinie %d pour le robot 1 non atteinte", numposition);
...
void Robot1::EnvoyerJoints(int joint1, int joint2, int joint3, int joint4, int joint5, int joint6, int joint7)
{
	...
	if(srv_simRosSetJoint.response.result==-1)
	{
		ROS_INFO("Position definie manuellement pour le robot 1 non atteinte");
...
void Robot1::DescendreBras()
{
	...
	if(srv_simRosSetJoint.response.result==-1)
	{
		ROS_INFO("Bras non descendu pour le robot 1");
	...
	{	
		...
		ROS_INFO("Bras descendu pour le robot 1");
...
void Robot1::MonterBras()
{
	...
	if(srv_simRosSetJoint.response.result==-1)
	{
		ROS_INFO("Bras non monte pour le robot 1");
	...
	{	
		...
		ROS_INFO("Bras monte pour le robot 1");
...
void Robot1::SendPositionCallback(const std_msgs::Int32::ConstPtr& msg)
{
...
void Robot1::SendJointsCallback(const robots::RobotJoints::ConstPtr& msg)
{
...
void Robot1::FermerPinceCallback(const std_msgs::Int32::ConstPtr& msg)
{
	FermerPince();
}
...
void Robot1::OuvrirPinceCallback(const std_msgs::Int32::ConstPtr& msg)
{
	OuvrirPince();	
}
...
void Robot1::DescendreBrasCallback(const std_msgs::Int32::ConstPtr& msg)
{
	DescendreBras();
}
...
void Robot1::MonterBrasCallback(const std_msgs::Int32::ConstPtr& msg)
{
	MonterBras();
}
...
void Robot1::ControlerRobotCallback(const robots::MoveRobot::ConstPtr& msg)
{
...
void Robot1::init(ros::NodeHandle noeud)
{
	...
	planifSendJoints = noeud.subscribe("/commande/Simulation/SendJointsRobot1",10,&Robot1::SendJointsCallback,this);
 	...
 	planifFermerPince = noeud.subscribe("/commande/Simulation/FermerPinceRobot1",10,&Robot1::FermerPinceCallback,this);

	planifOuvrirPince = noeud.subscribe("/commande/Simulation/OuvrirPinceRobot1",10,&Robot1::OuvrirPinceCallback,this);
	
	planifDescendreBras = noeud.subscribe("/commande/Simulation/DescendreBras1",10,&Robot1::DescendreBrasCallback,this);

	planifMonterBras = noeud.subscribe("/commande/Simulation/MonterBras1",10,&Robot1::MonterBrasCallback,this);

	planifControlerRobot = noeud.subscribe("/commande/Simulation/ControlerBras1",10,&Robot1::ControlerRobotCallback,this);


	//Publishers
	pub_pince = noeud.advertise<std_msgs::Int32>("/robot/cmdPinceRobot1", 10);
	
	pub_robotPosition = noeud.advertise<std_msgs::Int32>("/robot/PositionRobot1",10);
	pub_robotBras = noeud.advertise<std_msgs::Int32>("/robot/BrasRobot1",10);
	pub_robotPince = noeud.advertise<std_msgs::Int32>("/robot/PinceRobot1",10);

	pub_retourCommande = noeud.advertise<std_msgs::Int32>("/commande/Simulation/retourCommande1", 10);


	//Utilisation du service simRosGetObjectHandle pour obtenir les handles du robot 1
	for (int i=1;i<8;i++)
	{
		std::stringstream sr;	
		sr << i;
		...
		if(Rints[i-1]==-1)
		{
			ROS_INFO("Robot 1 : Handle non obtenu pour joint %d",i);
		}
		else
		{
			if(i==7)
